...
func TestDeltaArenaPooling(t *testing.T) {
	t.Run("ArenaReuse", func(t *testing.T) {
		// The free-list (deltaArenaFreeList) is process-global and FIFO, so any
		// arena left queued by an earlier test would make get→put→get return a
		// different arena than the one just put. Drain it first so this
		// reuse assertion observes a known-empty pool regardless of test order.
		for {
			select {
