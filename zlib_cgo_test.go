...
// TestLibdeflate_EmptyOutputContract pins the documented behavior that a
// zero-length destination returns (0, nil) without touching the source.
func TestLibdeflate_EmptyOutputContract(t *testing.T) {
	n, err := inflateZlibOneShot([]byte{0x78, 0x9c}, nil)
	require.NoError(t, err)
	require.Zero(t, n)
}
